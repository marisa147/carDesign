...
| 01-02-01 | 02 | 1 | FOUND-02, FOUND-04 | T-01-01 / T-01-03 | API settings parse env values, reject unsafe CORS, and never log secret values | unit/static | `uv run ruff check . && uv run mypy src && uv run pytest` from `services/api` | No, W0 creates API project | pending |
| 01-02-02 | 02 | 1 | FOUND-01, FOUND-02 | T-01-02 | Worker imports only worker modules and boots with Redis broker config placeholders | unit/static | `uv run pytest` from `services/worker` | No, W0 creates worker project | pending |
| 01-03-01 | 03 | 2 | FOUND-03 | T-01-02 / T-01-04 | Frontend consumes generated contract package only, not backend internals | contract/unit | `pnpm contracts:generate && pnpm contracts:check` | No, W0 creates contracts package | pending |
| 01-04-01 | 04 | 2 | FOUND-01, FOUND-04 | T-01-01 / T-01-05 | Docker Compose exposes local Postgres, Redis, and MinIO with documented local-only defaults | smoke/config | `pnpm infra:up && pnpm smoke:local` | No, W0 creates Compose file | pending |
| 01-05-01 | 05 | 3 | FOUND-02, FOUND-03 | T-01-02 / T-01-04 | Web shell calls the generated health client and exposes no provider keys | unit/static | `pnpm lint && pnpm typecheck && pnpm test` from `apps/web` | No, W0 creates web app | pending |
| 01-06-01 | 06 | 4 | FOUND-01, FOUND-02, FOUND-03, FOUND-04 | T-01-01 / T-01-02 / T-01-03 / T-01-04 / T-01-05 | Aggregate validation proves local run, validation, contract drift, and configurable env paths | integration | `pnpm validate` | No, W0 creates aggregate docs/scripts | pending |
...
- [ ] `packages/contracts/orval.config.ts` and contract generation/check scripts exist.
- [ ] `infra/compose.yml` and `pnpm smoke:local` exist for local service validation.
