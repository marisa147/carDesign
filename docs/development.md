...
| Path | Owner | Purpose |
| ---- | ----- | ------- |
| `apps/web` | Product plane | Next.js workbench, generated contract consumption, chat-to-brief flow, template catalog selection, structured and itasha parameters, assets, progress, 2D preview, PreviewSpec overlays/safe zones, 3D fallback, history, iteration, feedback, preflight, concept export, and future gates. |
| `services/core` | Shared data core | SQLAlchemy metadata, async database helpers, template governance/validation, production-readiness preflight helpers, and durable-data services shared by API and worker. |
| `services/api` | Control plane | FastAPI settings, `/health`, product/generation/template/preflight routes, Alembic migrations, OpenAPI export, and smoke scripts. |
| `services/worker` | Work plane | Celery entrypoint, Redis broker settings, health task, local no-provider simulation, and template-aware text-to-2D generation task. |
...
Template package validation is run from `services/core`:
...
It validates the five V3 MVP templates, required asset slots, PNG dimensions, mask bounds, safe zones, metadata, readiness, and legacy alias behavior.
...
Future gates are intentional product boundaries. Production UV-mapped 3D preview, print-ready export, marketplace/community flows, payment/order flows, hosted provider production rollout, auth, billing, and production handoff must be disabled, experimental, or documented as deferred until later phases implement and verify them.
...
## Phase 12 Lightweight 3D Preview

Phase 12 keeps default validation local, provider-off, and free. Enable the lightweight preview in local service env with `V2_LIGHTWEIGHT_3D_PREVIEW_ENABLED=true`; hosted image-provider flags are not required. The MVP registers one shell fixture, `generic-side-coupe-lightweight-v1`, for compatible `generic-side-coupe` side-view PreviewSpec templates.

The workbench `3D 预览` tab is client-only and uses Three.js for the lightweight shell. It supports rotate, zoom, reset camera, screenshot capture, visible `非生产贴膜参考` labeling, and fallback copy when no compatible shell exists. Screenshot capture creates immutable `preview_3d_screenshot` artifacts linked to the selected version and stores Preview3DSpec/camera/warning metadata; metadata must not contain binary image data, secrets, bearer tokens, or raw local paths.

The viewer is concept-only. It does not prove vehicle-specific UV mapping, print scale, bleed, DPI, color profile, physical installability, or print-ready wrap output. If a template is unsupported, the 2D PreviewSpec, generation, targeted edits, references, version history, and export flows must remain usable.

Focused Phase 12 provider-off checks:
...
## Phase 14 V2 MVP Release Hardening

Phase 14 is the V2 MVP release validation and documentation pass. It does not add production claims; it proves the implemented V2 MVP flows have a fresh local baseline, Docker smoke evidence, hosted-provider manual smoke instructions, desktop/mobile Browser UAT evidence, and a feature flag reference.

The release command set is:
...
Docker smoke uses `corepack pnpm infra:up` and `corepack pnpm smoke:local` to prove PostgreSQL, Redis, MinIO, Alembic, durable data, and local deterministic generation. The non-dry-run worker smoke requires prepared Docker/API/worker processes and should only be counted when `corepack pnpm smoke:worker` passes against those live services.

Hosted-provider smoke is manual-only and reversible. Use `.planning/phases/14-v2-mvp-hardening-docs-smoke-and-uat/14-HOSTED-SMOKE-RUNBOOK.md`, keep real keys in ignored env files, require operator cost approval, cap `AI_HOSTED_DAILY_CALL_LIMIT`, `AI_HOSTED_RATE_LIMIT_PER_MINUTE`, and `AI_MAX_ESTIMATED_COST_PER_JOB`, record one job/model-run/version/artifact/cost evidence bundle, then disable hosted flags.

The feature flag reference is `.planning/phases/14-v2-mvp-hardening-docs-smoke-and-uat/14-FEATURE-FLAGS.md`. It maps `V2_HOSTED_PROVIDER_ROLLOUT_ENABLED`, `V2_TARGETED_REGENERATION_ENABLED`, `V2_REFERENCE_GUIDANCE_ENABLED`, `V2_LIGHTWEIGHT_3D_PREVIEW_ENABLED`, `V2_ENHANCED_HANDOFF_PACKAGE_ENABLED`, `NEXT_PUBLIC_V2_*`, provider variables, hosted quota guards, local defaults, hosted smoke overrides, and rollback steps. Browser public flags are hints only; API/worker flags and guardrails remain authoritative.

Browser UAT evidence is stored in `.planning/phases/14-v2-mvp-hardening-docs-smoke-and-uat/14-HUMAN-UAT.md` and `evidence/`. It should cover desktop/mobile hosted guard visibility, targeted edit controls and comparison, reference warnings, lightweight 3D concept-only labels, enhanced handoff ZIP preview/history, and no horizontal overflow.

V2 MVP release notes and final verification must keep the same boundary language: concept-only, not print-ready, no verified production UV, no production wrap output, no auth, no billing, no marketplace/community flow, no quotes/orders/payments, no installer workflow, and no production deployment.
...
## Phase 12 Lightweight 3D Preview UAT
...
1. Set `V2_LIGHTWEIGHT_3D_PREVIEW_ENABLED=true` in the relevant local service env before starting API/web paths that need the flag.
2. Create or resume a workspace and generate or seed a version with `PreviewSpec` metadata for a compatible side-view template.
3. Select the generated version and open `3D 预览`.
4. Confirm the lightweight shell is nonblank and shows `概念 3D 预览` plus `非生产贴膜参考`.
5. Use rotate left/right, zoom out/in, reset camera, and screenshot controls; confirm the controls are keyboard reachable and do not overlap on narrow screens.
6. Confirm source artifact id/object key, shell id, camera preset, UV warning, safe-zone chips, and overlay chips are visible without exposing secrets or local paths.
7. Capture a 3D screenshot and confirm a `preview_3d_screenshot` artifact is linked to the selected version with concept-only and UV-not-verified warning metadata.
...
This UAT covers Phase 7 operational visibility, cancellation, and local worker smoke. It does not prove hosted provider production readiness, hosted image quality, commercial rights, auth, billing, production deployment, production UV-mapped 3D, or print-ready wrap output.
...
9. Confirm missing-rights reference/logo assets are not used for generation and future production UV-mapped 3D, production handoff, print-ready export, broad template library, and marketplace/community gates remain disabled or deferred.
...
8. Confirm production UV-mapped 3D, production handoff, print preflight, layered source export, and marketplace/community gates remain disabled or deferred.
...
8. Confirm production UV-mapped 3D, print-ready export, marketplace/community, and production handoff controls are disabled, experimental, or explicitly deferred.
...
This UAT does not cover the full GPT-style workbench, upload manager, rich 2D preview controls, export UX, production handoff, production UV-mapped 3D, auth, billing, quotas, or hosted provider quality.
...
| UI-07 | Production UV-mapped 3D, print-ready export, marketplace/community, and production handoff are disabled, experimental, or explicitly deferred. | Run `corepack pnpm --filter @caragent/web test` and Phase 4 Workbench UAT. |
...
| V2-3D-01 | At least one lightweight preview shell is registered and linked to existing vehicle templates and PreviewSpec. | Run core generation job tests, web compatibility tests, and Phase 12 verification. |
| V2-3D-02 | Users can open a 3D preview panel for a selected 2D design version. | Run web page tests and Phase 12 desktop/mobile UAT. |
...
## V3 Template Library And Production Readiness

V3 closes the template library and production-readiness milestone from `MVP_FINAL.md`. It is a concept-design milestone, not a production wrap handoff. The V3 release adds source/license governance, internal-original MVP template assets, catalog selection, selected-template traceability, concept-only production readiness preflight, enhanced handoff evidence, and release hardening.

Use this command set for a fresh V3 baseline:
...
For Browser UAT, start the API and web against a deterministic fixture, then verify desktop and mobile:

- Template catalog lists all five internal-original MVP templates.
- The selected template id/view/source/license/readiness are visible.
- 2D PreviewSpec, safe zones, warnings, and targeted edit controls are tied to the selected template.
- Unsupported 3D templates show explicit non-production 2D fallback.
- Enhanced handoff history includes `production-readiness-preflight.json` and `template-validation.json`.
- Production readiness preflight reports missing production evidence instead of claiming print readiness.
- No horizontal overflow appears at desktop or mobile widths.

Phase 20 evidence lives in `.planning/phases/20-v3-hardening-docs-smoke-and-uat/`.

V3 still defers print-ready PSD/AI/PDF export, verified scale/bleed/color/DPI, verified UV mapping, real licensed vehicle-specific template libraries, marketplace/order/payment/installer flows, automated legal licensing, and hosted-provider production-readiness claims.
...
| Phase 14 goal | Fresh aggregate validation, contracts, V1 compatibility, migration safety, Docker smoke, hosted-provider smoke runbook, Browser UAT evidence, feature flag reference, release notes, and final traceability prove the V2 MVP release baseline without production claims. |
| Phase 15 goal | Template source policy, license metadata, readiness reports, prohibited-source guards, audit output, docs, and validation prove reusable template provenance before catalog exposure. |
| Phase 16 goal | Five internal-original MVP templates, deterministic package assets, thumbnails, safe zones, masks, validation command, resolver integration, and regression tests prove the template pack without unauthorized vehicle imagery. |
| Phase 17 goal | Template catalog API, thumbnail serving, Workbench selector, source/license/readiness warnings, selected-template persistence, contracts, and tests prove deliberate template selection. |
| Phase 18 goal | Local deterministic generation on every MVP template, selected-template PreviewSpec overlays, targeted edit safe zones, reference/provider trace, 3D compatibility/fallback, and contract checks prove template-aware generation and review. |
| Phase 19 goal | Production readiness preflight schema, version-scoped API, durable preflight JSON exports, Workbench preflight UI, enhanced handoff template/preflight evidence, and export block tests prove concept-only readiness reporting. |
| Phase 20 goal | Aggregate validation, template-pack validation, V1 compatibility, migration safety, Docker/local smoke, worker dry run, Browser UAT, docs, release notes, and milestone audit prove the V3 release baseline. |
...
| D-28 | Phase 14 V2 MVP release evidence is provider-off by default; hosted-provider smoke remains manual-only, quota/cost guarded, secret-safe, and reversible. |
| D-29 | Phase 15 template governance blocks prohibited web-crawled and third-party-reference-only sources from reusable templates, masks, thumbnails, and catalog entries. |
| D-30 | Phase 16 template assets are internal-original generic MVP fixtures and must validate locally before catalog or generation use. |
| D-31 | Phase 17 template selection persists through brief/job/export state instead of living only in frontend state. |
| D-32 | Phase 18 PreviewSpec remains the selected-template review contract across 2D overlays, targeted edits, references, 3D fallback, and compatibility checks. |
| D-33 | Phase 19 production readiness preflight is concept-only and records missing evidence; it does not unlock print-ready PSD/AI/PDF export. |
| D-34 | Phase 20 V3 release evidence is provider-off by default and requires aggregate validation, template-pack validation, Docker/local smoke, Browser UAT, docs, and audit before archive. |

Deferred items after Phase 20 remain out of scope until later milestones implement them: print-ready PSD/AI/PDF export, verified scale/bleed/color/DPI, verified vehicle-specific UV mapping, production handoff, real licensed vehicle-specific template coverage, auth, billing, marketplace/community flows, automated copyright/licensing verification, quotes/orders/payments, installer workflows, and production deployment. Hosted output quality, pricing, moderation, account status, mask/reference-image support, and commercial terms must still be rechecked before any non-local output is treated as production-ready evidence.
...
- `NEXT_PUBLIC_V2_*` browser flags are public hints only; server-side `V2_*` flags, provider capability metadata, rights/source checks, quota guards, and API/worker validation remain authoritative.
...
- Reusable template assets, masks, thumbnails, and catalog entries must come from allowed source types only; web-crawled and third-party-reference-only material must remain reference-only and fail closed.
- Production readiness preflight is an evidence-gap report, not a production approval. Missing scale, bleed, color profile, DPI, UV mapping, installer notes, or real licensed-template proof must keep print-ready export blocked.
...
Phase 20 is complete when the foundation, durable data/job/asset surfaces, structured brief/prompt trace, local deterministic generation task, API generation routes, integrated web workbench, selected-version iteration, lineage comparison, feedback, concept export manifest, itasha controls, PreviewSpec metadata, safe-zone overlays, operations API, structured failures, cancellation, retry/fallback settings, hosted provider capability map, BFL adapter, hosted preflight, provider trace/cost/failure diagnostics, workbench provider selector, targeted edit schemas, mask preview, deterministic recomposition, provider-mask guardrails, retry-safe targeted failures, comparison UI, six-role reference assignments, rights/source snapshots, provider reference warnings, durable reference trace metadata, child-iteration reference reuse, lightweight 3D preview contracts/viewer/screenshot artifacts, enhanced concept handoff ZIPs, template source governance, five internal-original MVP templates, template catalog selection, selected-template traceability, concept-only production readiness preflight, provider-off validation, template-pack validation, Docker smoke, hosted smoke runbooks, Browser UAT evidence, feature flag reference, release notes, milestone audit, and final traceability can be run or inspected from documented commands. Production-ready wrap output, print-ready PSD/AI/PDF packages, verified production preflight, verified UV-mapped 3D, real licensed vehicle-specific template marketplace, auth, billing, marketplace/community flows, automated licensing verification, quotes/orders/payments, installer workflows, and production deployment remain deferred to later phases.
